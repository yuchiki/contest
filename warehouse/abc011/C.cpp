...
bool reach[100 + 1][MAXN + 1];
int ng1, ng2, ng3;

void ngize(int phase) {
  reach[phase][ng1] = false;
  reach[phase][ng2] = false;
  reach[phase][ng3] = false;
}

signed main() {
  int N = readint();
  cin >> ng1 >> ng2 >> ng3;
  repinc(i, 100) repinc(j, MAXN) reach[i][j] = false;
  reach[0][N] = true;
  ngize(0);
  rep(i, 100) {
    repinc(j, MAXN) {
      if (!reach[i][j]) continue;
      if (j - 1 >= 0) reach[i + 1][j - 1] = true;
      if (j - 2 >= 0) reach[i + 1][j - 2] = true;
      if (j - 3 >= 0) reach[i + 1][j - 3] = true;
    }
    ngize(i + 1);
  }

  repinc(i, 100) if (reach[i][0]) exityes();
  sayno();
  return 0;
}
